...
function PlotAverageActionsOverTime(l::Dict, numT::Int64, I::Int64, B::Int64, W::Int64, A::Int64, V::Int64, actionsMap::Dict)
    ...
    for i in 1:n
        ...
        if numT == 10
            ...
            highvol = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[4] >= 7 && x[1] > 0, states)])
            lowvol = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[4] <= 4 && x[1] > 0, states)])

            highspr = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[3] >= 6 && x[1] > 0, states)])
            lowspr = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[3] <= 4 && x[1] > 0, states)])

            hightime = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[1] >= 7 && x[1] > 0, states)])
            lowtime = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[1] <= 4 && x[1] > 0, states)])

            highinv = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[2] >= 7 && x[1] > 0, states)])
            lowinv = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[2] <= 4 && x[1] > 0, states)])

            highvol_lowspr = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[4] >= 7 && x[3] <= 4 && x[1] > 0, states)]) 
            lowvol_highspr = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[4] <= 4 && x[3] >= 6 && x[1] > 0, states)]) 

            hightime_highinv = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[1] >= 7 && x[2] >= 7 && x[1] > 0, states)]) 
            lowtime_lowinv = getindex.(Ref(l[i]["rlAgent_1"]["Q"]), states[findall(x -> x[1] <= 4 && x[2] <= 4 && x[1] > 0, states)]) 
    ...
    savefig(v, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyAverageActionVolume_V" * string(V) * "_S" * string(numT) * "_430.pdf")
    ...
    savefig(s, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyAverageActionSpread_V" * string(V) * "_S" * string(numT) * "_430.pdf")
    ...
    savefig(t, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyAverageActionTime_V" * string(V) * "_S" * string(numT) * "_430.pdf")
    ...
    savefig(i, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyAverageActionInventory_V" * string(V) * "_S" * string(numT) * "_430.pdf")
    ...
    savefig(sv, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyAverageActionSpreadVolume_V" * string(V) * "_S" * string(numT) * "_430.pdf")
    ...
    savefig(ti, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyAverageActionTimeInventory_V" * string(V) * "_S" * string(numT) * "_430.pdf")
...
# numT = I = B = W = 5                    # number of time, inventory, spread, volume states 
# V = 100
# @time l = load(path_to_files * "Data/RL/Training/ResultsTest_Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25.jld")["rl_results"]
# n = length(l)
...
function PlotVolumeTragectory(l::Dict, numT::Int64, I::Int64, B::Int64, W::Int64, A::Int64, V::Int64, actionsMap::Dict)
    ...
    savefig(p, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyActionVolumeIteration_" * string(n) * "_V" * string(V) * "_S" * string(numT) * "_430.pdf")
end
# A = 9                          # number of action states (if odd TWAP price will be an option else it will be either higher or lower)
# maxVolunmeIncrease = 2.0       # maximum increase in the number of TWAP shares (fix at 2 to make sure there are equal choices to increase and decrease TWAP volume)
# actions = GenerateActions(A, maxVolunmeIncrease)
...
# @time l = load(path_to_files * "Data/RL/Training/MARL/Results_Buy_1_Sell_0_alpha0.1_lambda0.003_gamma0.25.jld")["rl_results"]
# rewards = [l[i]["rlAgent_1"]["TotalReward"] for i in 1:1000]
# rewards = rewards[findall(x -> abs(x) < 500000, rewards)]
...
# savefig(p, path_to_files * "/Images/RL/Z_Results_Buy_1_Sell_0_alpha0.1_lambda0.003_gamma0.25/BuyReward.pdf")

# num_trades = [l[i]["rlAgent_1"]["NumberTrades"] for i in 1:200]
...
# # hline!(p, [(-10000 + 10100) * 43000], color = :black)
# savefig(p, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyNumTrades.pdf")

# rem_inventory = [l[i]["rlAgent_1"]["RemainingInventory"] for i in 1:200]
# p = plot(rem_inventory, fillcolor = :red, linecolor = :red, legend = :outertopright, xlabel = "Episodes", legendfontsize = 4, fg_legend = :transparent, fontfamily="Computer Modern")
# savefig(p, path_to_files * "/Images/RL/Slippage_BuySell_ExpTIDivXPenalty_RLOutVWAP_E200_alpha0.5_lambda0.0005_gamma0.25/BuyRemainingInventory.pdf")
