...
function Calibrate(initialsolution::Vector{Float64}, empiricallogreturns::Vector{Float64}, empiricalmoments::Moments; f_reltol::Vector{Float64} = [0.3, 0.2, 0.1, 0], ta_rounds::Vector{Int64} = [4, 3, 2, 1], neldermeadstate = nothing)
    ...
    try
        ...
        optimizationoptions = Options(show_trace = true, store_trace = true, trace_simplex = true, extended_trace = true, iterations = 13, ξ = 0.15, ta_rounds = ta_rounds, f_reltol = f_reltol)
        ...
        save("../Data/Calibration/OptimizationResult1.jld", "result", result)
        println(run(`free -m`))
...
initialState = load("../Data/Calibration/OptimizationResult.jld")["result"]
neldermeadstate = initialState.end_state
initialsolution = minimizer(initialState)

ta_rounds_arg = [60, 45, 35, 30, 20, 10]
f_reltol_arg = [0.4, 0.3, 0.2, 0.1, 0.05, 0]
...
# savefig(objectives, "../Images/Calibration/NMTAFitnessBestVertex3.pdf")
# # Simplex values
...
# savefig(convergence, "../Images/Calibration/NMTAFitnessAllSimplexValues3.pdf")
#---------------------------------------------------------------------------------------------------