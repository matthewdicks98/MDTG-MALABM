...
function StylizedFacts(exchange::String, l1lobPath::String, startTime::DateTime, endTime::DateTime; format::String = "pdf")
    ...
    LogReturnDistribution(exchange, suffix, logreturns; format = format)
    println("Log-return distribution complete")
    LogReturnAutocorrelation(exchange, suffix, logreturns, 500; format = format)
    println("Log-return auto-correaltion complete")
    TradeSignAutocorrelation(exchange, suffix, data, 500; format = format)
    println("Trade sign auto-correaltion complete")
    ExtremeLogReturnPercentileDistribution(exchange, suffix, logreturns; format = format)
...
function LogReturnDistribution(exchange::String, suffix::String, logreturns::Vector{Float64}; format::String = "pdf")
    ...
    savefig(distribution, string("../Images/" * exchange * "/Log-ReturnDistribution" * suffix * ".", format))
end
#---------------------------------------------------------------------------------------------------

#----- Log-return and absolute log-return autocorrelation -----#
function LogReturnAutocorrelation(exchange::String, suffix::String, logreturns::Vector{Float64}, lag::Int64; format::String = "pdf")
    color = exchange == "CoinTossX" ? :green : (exchange == "JSE" ? :purple : :orange)
    ...
    savefig(autoCorrPlot, string("../Images/" * exchange * "/Log-ReturnAutocorrelation" * suffix * ".", format))
end
#---------------------------------------------------------------------------------------------------

#----- Trade sign autocorrealtion -----#
function TradeSignAutocorrelation(exchange::String, suffix::String, data::DataFrame, lag::Int64; format::String = "pdf")
    color = exchange == "CoinTossX" ? :green : (exchange == "JSE" ? :purple : :orange)
    ...
    savefig(autoCorrPlot, string("../Images/" * exchange * "/Trade-SignAutocorrelation" * suffix * ".", format))
end
#---------------------------------------------------------------------------------------------------

#----- Extreme log-return percentile distribution for different time resolutions -----#
function ExtremeLogReturnPercentileDistribution(exchange::String, suffix::String, logreturns::Vector{Float64}; format::String = "pdf")
    color = exchange == "CoinTossX" ? :green : (exchange == "JSE" ? :purple : :orange)
    ...
    savefig(extremePercentileDistributionPlot, string("../Images/" * exchange * "/ExtremeLog-ReturnPercentilesDistribution" * suffix * ".", format))
end
#---------------------------------------------------------------------------------------------------
...
function DepthProfile(exchange::String, depthProfilePath::String; format::String = "pdf")
    ...
    suffix = ""
    depthProfilePath == "DepthProfileData" ? suffix = "" : suffix = depthProfilePath[17:end]
    savefig(depthProfile, string("../Images/" * exchange * "/DepthProfile" * suffix * ".", format))
...
function PriceImpact(exchange::String, l1lobPath::String, startTime::DateTime, endTime::DateTime; format::String = "pdf")
    ...
    suffix = ""
    l1lobPath == "L1LOB" ? suffix = "" : suffix = l1lobPath[6:end]
    savefig(priceImpact, string("../Images/" * exchange * "/PriceImpact" * suffix * ".", format))
